...
export function validateBrandPayload(body) {
	...
	const logo = assetPath(b.logo ?? '/images/combatmaster-hacks-logo.webp');
	const logoRaster = assetPath(b.logoRaster ?? '/images/combatmaster-hacks-logo.png');
	const defaultOgImage = assetPath(b.defaultOgImage ?? '/images/combatmaster-hacks-combat.webp');
	const heroImage = assetPath(b.heroImage ?? '/images/combatmaster-hero-banner.webp');
...
export function readBrandForStudio() {
	...
	const keywords = listMatch
		...
		: [];
	const seoBlock = matchBlock(src, 'seo');
	const copyBlock = matchBlock(src, 'copy');
	const sitemapBlock = matchBlock(src, 'sitemap');
	const themeBlock = matchBlock(src, 'theme');
	/** @type {Record<string, string>} */
	const seo = {};
