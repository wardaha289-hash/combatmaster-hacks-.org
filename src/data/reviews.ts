...
import { absoluteImageUrl, crawlPhotoMeta, reviewsHeroImageSrc } from './page-images';
...
export function getReviewCrawlImage(slug: string, handle: string, caption: string) {
	return crawlPhotoMeta(slug, `Combat Master Hacks review by @${handle}`, caption);
}
...
export function getReviewSitemapEntries() {
	...
	const indexImage = {
		url: absoluteImageUrl(reviewsHeroImageSrc),
