...
const icons: Record<string, string> = {
	home: 'M4.5 10.5L12 4.5l7.5 6M6 9.8V18a1 1 0 001 1h3v-4h4v4h3a1 1 0 001-1V9.8',
	hacks:
	...
	forums:
		'M5 6.5h14a1.2 1.2 0 011.2 1.2v8.6a1.2 1.2 0 01-1.2 1.2H5A1.2 1.2 0 013.8 16.3V7.7A1.2 1.2 0 015 6.5zM8 10h8M8 13h5',
};
...
function NavbarInner({
	...
	const [gunPopId, setGunPopId] = useState<string | null>(null);
	...
	const triggerGunPop = (id: string) => {
		setGunPopId(id);
		window.setTimeout(() => setGunPopId(null), 620);
	};
	...
	const renderNavLink = (item: (typeof navLinks)[number], onNavigate?: () => void) => {
		const popping = gunPopId === item.id;
		const className = [
			'site-nav__bubble',
			item.active ? 'is-active' : undefined,
			popping ? 'is-popping' : undefined,
		]
			.filter(Boolean)
			.join(' ');
		...
		return (
			<a
				...
				href={item.href}
				className={className}
				onPointerDown={() => triggerGunPop(item.id)}
				onClick={onNavigate}
			>
				{popping ? (
					<span className="site-nav__gun-burst" aria-hidden="true">
						<M416GunIcon className="site-nav__gun-icon" />
					</span>
				) : null}
				...
				<div className="site-panel" id="site-nav-panel">
					<div className="shell site-panel__inner">
						<nav className="site-panel__nav" aria-label={t('nav.mobileAria')}>
							{navLinks.map((item) => renderNavLink(item, () => setOpen(false)))}
						</nav>
