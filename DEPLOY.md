# Deploy combatmasterhacks.org

Step-by-step guide to deploy the Combat Master Hacks static site to **combatmasterhacks.org** on Cloudflare Pages, configure DNS, and submit to Google Search Console.

## Prerequisites

- Node.js **≥ 22.12.0**
- Cloudflare account with access to **combatmasterhacks.org** DNS
...
3. Configure build settings:
   - **Project name:** `combatmasterscheats` (existing) or create a new project
...
Add **combatmasterhacks.org** as the primary custom domain on the Pages project.

### Apex (combatmasterhacks.org)
...
2. In **Rules** → **Redirect Rules** (or Bulk Redirects), create:
   - **Source:** `www.combatmasterhacks.org/*`
   - **Target:** `https://combatmasterhacks.org/${1}`
   - **Status:** 301

The deployed `functions/_middleware.js` also enforces apex canonical host, legacy domain redirects (`combatmasterhacks.org`, `.net`, `.com`), and legacy path redirects.
...
- `https://combatmasterhacks.org/`
- `https://combatmasterhacks.org/es/`
- `https://combatmasterhacks.org/combatmaster-hacks/`
- `https://combatmasterhacks.org/combatmaster-aimbot/`
- `https://combatmasterhacks.org/sitemap.xml`
- `https://combatmasterhacks.org/robots.txt`

Verify redirects:

- `http://combatmasterhacks.org` → `https://combatmasterhacks.org` (301)
- `https://www.combatmasterhacks.org` → `https://combatmasterhacks.org` (301)
- Legacy domains (e.g. `combatmasterhacks.org`) → `https://combatmasterhacks.org` (301)
...
2. **Add property** → choose **Domain** → enter `combatmasterhacks.org`.
...
4. After verification, open **Sitemaps** and submit:
   ...
   https://combatmasterhacks.org/sitemap.xml
   ```
   Remove any legacy submissions (`sitemap-index.xml`, old `combatmasterhacks.org` URLs).
5. Use **URL Inspection** to request indexing for:
   - Homepage (`/`)
   - Pillar page (`/combatmaster-hacks/`)
   - Key landing pages (`/combatmaster-aimbot/`, `/combatmaster-esp/`, `/combatmaster-hacks-2026/`, etc.)
...
- [ ] Custom domain `combatmasterhacks.org` attached and active
- [ ] `www` redirects to apex
- [ ] Legacy domains 301 to `combatmasterhacks.org`
- [ ] Always Use HTTPS enabled
- [ ] `robots.txt` and sitemaps serve from `https://combatmasterhacks.org`
- [ ] Google Search Console domain verified
- [ ] `sitemap.xml` submitted in GSC
- [ ] Homepage and `/combatmaster-hacks/` requested for indexing
